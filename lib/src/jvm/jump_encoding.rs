...
use crate::jvm::model::{BasicBlock, SynLabel, SynLabelGenerator};
...
pub fn widen_oversized_jumps<Frame: Clone, Range: RangeBounds<isize>>(
    ...
    // relative offsets
    let mut rewritable_jumps: Vec<JumpInterval> = blocks
        .iter()
    ...
    let mut oversized_jumps: Vec<RefId<JumpInterval>> = rewritable_jumps
        .iter()
...
mod test {
    use super::*;
    use crate::jvm::{EqComparison, OrdComparison};
