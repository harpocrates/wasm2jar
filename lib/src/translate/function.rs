...
use crate::jvm::{
    ...
    MethodDescriptor, OffsetVec, OrdComparison, RefType, Width, UnqualifiedName, BinaryName
};
use crate::wasm::{
...
pub struct FunctionTranslator<'a, 'wasm: 'a, 'jvm: 'a, B: CodeBuilder<'jvm> + Sized, R> {
    /// WASM type of the function being translated
    function_typ: FunctionType,

    /// Translation settings
    settings: &'a Settings<'jvm>,

    /// Utilities
    utilities: &'a mut UtilityClass<'jvm>,

    /// Code builder
    jvm_code: &'a mut B,

    /// Tables
    wasm_tables: &'a [Table<'jvm>],

    /// Globals
    wasm_globals: &'a [Global<'wasm, 'jvm>],

    /// Local variables
    jvm_locals: LocalsLayout<'jvm>,
    ...
    wasm_function: FunctionBody<'wasm>,
...
impl<'a, 'wasm: 'a, 'jvm: 'a, B, R> FunctionTranslator<'a, 'wasm, 'jvm, B, R>
where
    B: CodeBuilderExts<'jvm> + Sized,
    R: WasmModuleResourcesExt,
{
    ...
    fn visit_operator(
        ...
        match operator {
            ...
            Operator::I32Ctz => self
                .jvm_code
                .invoke(&BinaryName::INTEGER, &UnqualifiedName::NUMBEROFTRAILINGZEROS)?,
            Operator::I32Popcnt => self.jvm_code.invoke(&BinaryName::INTEGER, &UnqualifiedName::BITCOUNT)?,
            ...
            Operator::I32Rotl => self.jvm_code.invoke(&BinaryName::INTEGER, &UnqualifiedName::ROTATELEFT)?,
            Operator::I32Rotr => self.jvm_code.invoke(&BinaryName::INTEGER, &UnqualifiedName::ROTATERIGHT)?,
            ...
            Operator::I64Popcnt => {
                self.jvm_code.invoke(&BinaryName::LONG, &UnqualifiedName::BITCOUNT)?;
                self.jvm_code.push_instruction(I2L)?;
            }
            ...
            Operator::I64Rotl => {
                self.jvm_code.push_instruction(L2I)?;
                self.jvm_code.invoke(&BinaryName::LONG, &UnqualifiedName::ROTATELEFT)?;
            }
            Operator::I64Rotr => {
                self.jvm_code.push_instruction(L2I)?;
                self.jvm_code.invoke(&BinaryName::LONG, &UnqualifiedName::ROTATERIGHT)?;
            }
            ...
            Operator::F32Ceil => {
                ...
                self.jvm_code.invoke(&BinaryName::MATH, &UnqualifiedName::CEIL)?;
                self.jvm_code.push_instruction(D2F)?;
            }
            Operator::F32Floor => {
                self.jvm_code.push_instruction(F2D)?;
                self.jvm_code.invoke(&BinaryName::MATH, &UnqualifiedName::FLOOR)?;
                self.jvm_code.push_instruction(D2F)?;
            }
            Operator::F32Trunc => {
                self.utilities
            ...
            Operator::F32Nearest => {
                self.jvm_code.push_instruction(F2D)?;
                self.jvm_code.invoke(&BinaryName::MATH, &UnqualifiedName::RINT)?;
                self.jvm_code.push_instruction(D2F)?;
            }
            Operator::F32Sqrt => {
                self.jvm_code.push_instruction(F2D)?;
                self.jvm_code.invoke(&BinaryName::MATH, &UnqualifiedName::SQRT)?;
                self.jvm_code.push_instruction(D2F)?;
            }
            ...
            Operator::F32Min => self.jvm_code.invoke(&BinaryName::FLOAT, &UnqualifiedName::MIN)?,
            Operator::F32Max => self.jvm_code.invoke(&BinaryName::FLOAT, &UnqualifiedName::MAX)?,
            ...
            Operator::F64Ceil => self.jvm_code.invoke(&BinaryName::MATH, &UnqualifiedName::CEIL)?,
            Operator::F64Floor => self.jvm_code.invoke(&BinaryName::MATH, &UnqualifiedName::FLOOR)?,
            Operator::F64Trunc => {
                self.utilities
                    .invoke_utility(UtilityMethod::F64Trunc, self.jvm_code)?;
            }
            Operator::F64Nearest => self.jvm_code.invoke(&BinaryName::MATH, &UnqualifiedName::RINT)?,
            Operator::F64Sqrt => self.jvm_code.invoke(&BinaryName::MATH, &UnqualifiedName::SQRT)?,
            ...
            Operator::F64Min => self.jvm_code.invoke(&BinaryName::DOUBLE, &UnqualifiedName::MIN)?,
            Operator::F64Max => self.jvm_code.invoke(&BinaryName::DOUBLE, &UnqualifiedName::MAX)?,
    ...
    fn visit_call(&mut self, func_idx: u32) -> Result<(), Error> {
        ...
        //
        let class_name: BinaryName = BinaryName::try_from(format!(
            "{}${}0",
            self.settings.output_full_class_name, self.settings.part_short_class_name,
        ).as_str()).unwrap();
        let method_name: UnqualifiedName = UnqualifiedName::try_from(
            format!("{}{}", self.settings.wasm_function_name_prefix, func_idx).as_str()
        ).unwrap();
        {
            let mut class_graph = self.jvm_code.class_graph();
            let mut desc = func_typ.method_descriptor();
            desc.parameters.push(self.settings.module_field_type());
            class_graph
                .classes
                .get_mut(&class_name)
    ...
    fn pack_stack_into_array(&mut self, expected: &[StackType]) -> Result<(), Error> {
        ...
        for stack_value in expected.iter().rev() {
            ...
            let value_of = &UnqualifiedName::VALUEOF;
            match stack_value {
                StackType::I32 => self.jvm_code.invoke(&BinaryName::INTEGER, value_of)?,
                StackType::I64 => self.jvm_code.invoke(&BinaryName::LONG, value_of)?,
                StackType::F32 => self.jvm_code.invoke(&BinaryName::FLOAT, value_of)?,
                StackType::F64 => self.jvm_code.invoke(&BinaryName::DOUBLE, value_of)?,
...
struct LocalsLayout<'b> {
    ...
    ///
    jvm_locals: OffsetVec<FieldType<'b>>,

    /// Index into `jvm_locals` for getting the "this" argument
    jvm_module_idx: usize,
}

impl<'b> LocalsLayout<'b> {
    fn new(method_arguments: impl Iterator<Item = FieldType<'b>>, module_typ: RefType<'b>) -> Self {
    ...
    fn lookup_this(&self) -> Result<(u16, FieldType<'b>), Error> {
    ...
    fn lookup_local(&self, mut local_idx: u32) -> Result<(u16, FieldType<'b>), Error> {
    ...
    fn push_local(&mut self, field_type: FieldType<'b>) -> Result<u16, Error> {
        let next_local_idx =
    ...
    fn pop_local(&mut self) -> Result<(u16, FieldType<'b>), Error> {
        self.jvm_locals
            .pop()
